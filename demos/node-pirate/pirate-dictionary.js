...
module.exports.translateWord = function (word) {
	var pirateWord = this.dictionary[word.toLowerCase()];
	if (pirateWord === undefined) return word;
	else return this.applyCase(word, pirateWord);
}
...
module.exports.translate = function (text) {
	...
	for (var i = 0; i < text.length; i += 1) {
		...
		// should translate the current word and add it to the translation
			...
			if (word != "") {
				...
				var pirateWord = this.translateWord(word);
				console.log("\t'%s' -> '%s'", word, pirateWord);
	...
	if (word !== "") {		
		translatedText += this.translateWord(word);
		console.log("\t'%s' -> '%s'", word, this.translateWord(word));
	}
